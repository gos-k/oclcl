...
(define-kernel-module :diffuse1-eazyopencl)
(in-kernel-module :diffuse1-eazyopencl)
...
(defkernel diffusion2d (void ((f float*) (fn float*)
  ...
  (let ((local-id-x (to-int (get-local-id 0)))
        (local-id-y (to-int (get-local-id 1)))
        (global-id-x (to-int (get-global-id 0)))
        (global-id-y (to-int (get-global-id 1)))
        (local-size-x (to-int (get-local-size 0)))
        (local-size-y (to-int (get-local-size 1)))
        (group-id-x (to-int (get-group-id 0)))
        (group-id-y (to-int (get-group-id 1)))
        (num-groups-x (to-int (get-num-groups 0)))
        (num-groups-y (to-int (get-num-groups 1))))
    (let* ((jx (+ local-id-x 1))
           (jy (+ local-id-y 1))
